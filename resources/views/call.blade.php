...
<body>
    <div class="container">
        <h2 class="title">WebRTC Calling System (Pusher)</h2>
    ...
    <script>
        ...
        async function testSignaling() {
            ...
            // Test: Send a POST signal
            debugLog('1. Sending POST signal via Pusher...');
            try {
        ...
        class WebRTCApp {
            constructor() {
                ...
                debugLog('WebRTC App initialized with Pusher. User ID: ' + this.userId);
            ...
            async startCall(mode) {
                try {
                    ...
                    // Send join message via Pusher
                    await this.sendSignal('join', {
                        userId: this.userId
                    });
                    debugLog('Join signal sent via Pusher');
            ...
            async sendSignal(type, data) {
                try {
                    debugLog('Sending signal via Pusher: ' + type);
                    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                    ...
                    // Send to backend which will broadcast via Pusher
                    ...
                    debugLog('Signal sent via backend: ' + type);
